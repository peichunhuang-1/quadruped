...
class LegVelocityEstimation {
    public:
        ...
        STATE calculate(IMU_DATA imu, ENCODER_DATA m, DST_DATA d, bool update = false, , bool lidar_update = true) ;
        int que_size = 10;
        Leg leg;
        ...
        double durations ;
        std::deque<IMU_DATA> imus ;
        std::deque<ENCODER_DATA> encoders ;
        DST_DATA dst ;
        GND_DATA g;
        ...
        double gaussianLikelihood(double x, double m, double s) {
        ...
        double weight(Eigen::Vector3d position, bool update, double sigma = 0.01) {
            ...
            return gaussian_erf((dst + sigma)/ sqrt(2 * sigma * sigma)) - gaussian_erf((dst - sigma)/ sqrt(2 * sigma * sigma));
        }
};
