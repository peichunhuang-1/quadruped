...
STATE LegVelocityEstimation::calculate(IMU_DATA imu, ENCODER_DATA m, DST_DATA d, bool update, bool lidar_update) {
    STATE state;
    if (update) {
        STATE last_state;
        IMU_DATA last_imu;
        ...
        std::deque<STATE>().swap(states);
        states.push_back(state);
        std::deque<IMU_DATA>().swap(imus);
        std::deque<ENCODER_DATA>().swap(encoders);
        encoder_input(m);
        dst = d;
    }
    else {
