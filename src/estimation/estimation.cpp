...
int main(int argc, char* argv[]) {
    ...
    Eigen::MatrixXd estimate_state = Eigen::MatrixXd::Zero(n, 48);
    ...
    ContactMap cm;
    for (int i = 1; i < n - 1; i++) {
        bool update_lidar = counter % 40 == 0? true: false;
        bool update = counter % 10 == 0? true: false;
        // std::cout << i << "\n";
        ENCODER_DATA elf = {
            df.iloc("lf.beta", i),
            df.iloc("lf.theta", i),
        ...
        double alpha_l = atan2((df.iloc("lf.dist", i) + 2e-3 * random_number() - df.iloc("lh.dist", i) - 2e-3 * random_number()) , 0.4);
        double alpha_r = atan2((df.iloc("rf.dist", i) + 2e-3 * random_number() - df.iloc("rh.dist", i) - 2e-3 * random_number()) , 0.4);
        DST_DATA dlf = {
            df.iloc("lf.dist", i) + 2e-3 * random_number(),
            alpha_l
        };
        DST_DATA drf = {
            df.iloc("rf.dist", i) + 2e-3 * random_number(),
            alpha_r
        };
        DST_DATA drh = {
            df.iloc("rh.dist", i) + 2e-3 * random_number(),
            alpha_r
        };
        DST_DATA dlh = {
            df.iloc("lh.dist", i) + 2e-3 * random_number(),
            alpha_l
        };
        IMU_DATA imu = {
            Eigen::Vector3d(df.iloc("a.x", i), df.iloc("a.y", i), df.iloc("a.z", i)) + 3e-2 * random_vector<3>() + Eigen::Vector3d(1e-3, 1e-3, 1e-3),
            Eigen::Vector3d(df.iloc("w.x", i), df.iloc("w.y", i), df.iloc("w.z", i)) + 1e-2 * random_vector<3>() + Eigen::Vector3d(1e-4, 1e-4, 1e-4),
        ...
        fopt_p.prediction();
        fopt_p.update_weight(update_lidar);
        ...
        estimate_state.row(i).segment(36, 3) = lf_leg.leg.contact_velocity;
        estimate_state.row(i).segment(39, 3) = rf_leg.leg.contact_velocity;
        estimate_state.row(i).segment(42, 3) = rh_leg.leg.contact_velocity;
        estimate_state.row(i).segment(45, 3) = lh_leg.leg.contact_velocity;
        counter++;
    ...
    "lf.v.x", "lf.v.y", "lf.v.z", "rf.v.x", "rf.v.y", "rf.v.z", "rh.v.x", "rh.v.y", "rh.v.z", "lh.v.x", "lh.v.y", "lh.v.z"};
