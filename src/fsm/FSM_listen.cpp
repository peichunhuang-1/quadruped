...
int main()
{
    ...
    while(1) {
        ...
        if (gait_info.position().pose().size() == 4) {
            ...
            lfr.set_angle(lf_motors.first); lfl.set_angle(lf_motors.second);
            rfr.set_angle(rf_motors.first); rfl.set_angle(rf_motors.second);
            rhr.set_angle(rh_motors.first); rhl.set_angle(rh_motors.second);
            lhr.set_angle(lh_motors.first); lhl.set_angle(lh_motors.second);
