...
Eigen::Vector3d LegVelocityEstimation::velocity(IMU_DATA imu, ENCODER_DATA m, DST_DATA d) {
    ...
    if (current_rim != cm.lookup(encoders[length-1].theta, contact_beta)) {
        rolling -= (rot * leg.contact_point - last_contact_point);
    }
    ...
    if (! (current_rim && last_rim) ) differ_of_point = Eigen::Vector3d(0, 0, 0);
...
STATE LegVelocityEstimation::calculate(IMU_DATA imu, ENCODER_DATA m, DST_DATA d, bool update) {
    ...
    if (update) {
        ...
        Eigen::Vector3d current_position = predict_position(last_state, last_imu);
        state = {
            current_position,
            velocity(imu, m, d), 
            m.beta + d.alpha,
            ...
            // 1,
            ground_info(current_position, imu.q, d)
        };
        // state.weight = weight(state, imu, m, state.ground);
        ...
        encoder_input(m);
        Eigen::Vector3d current_position = predict_position(states.back(), imus.back());
        state = {
            current_position,
